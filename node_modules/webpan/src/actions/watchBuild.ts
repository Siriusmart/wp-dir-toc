...
interface FsDiffCreated {
    type: "created",
    fsPath: string,
    buffer: ["file", Buffer] | ["dir"],
    hash: string | null
}

interface FsDiffChanged {
    type: "changed",
    fsPath: string,
    buffer: ["file", Buffer] | ["dir"],
    hash: string | null
...
interface FsDiffRemoved {
    type: "removed",
}

type FsDiffEntry = FsDiffCreated | FsDiffChanged | FsDiffRemoved

export async function watchBuild(
    buildInstance: BuildInstance,
    fsContent: FsContentEntries,
    ...
    let diff: Map<string, FsDiffEntry> = new Map()
    ...
    watcher.on("all", async (event, filename) => {
        ...
        let fsPath = path.join(buildInstance.getRoot(), "src", filename as string);
        let childPath = path.join("/", filename as string)
        ...
        switch (event) {
            case "change": {
                ...
                if (s.isDirectory()) {
                    ...
                    let buffer: ["file", Buffer] = ["file", await fs.readFile(fsPath)]
        ...
        debounce = setTimeout(() => {
            ...
            let outDiff: DiffEntries<string> = new Map()
